...
#include "../../duilib/DuiLib/Layout/UIChildLayout.h"
#include <new>
...
struct duilib_childlayout {
    ...
    duilib_childlayout() {
        layout_ui = new(std::nothrow) CChildLayoutUI();
    }
...
static bool is_valid_child_layout(duilib_childlayout_t layout) {
...
//=============================================================================
// CChildLayoutUI C API implementation
//=============================================================================

DUILIB_C_API duilib_childlayout_t duilib_childlayout_create(void) {
    return new(std::nothrow) duilib_childlayout();
}
...
DUILIB_C_API bool duilib_childlayout_is_valid(duilib_childlayout_t layout) {
    return is_valid_child_layout(layout);
}
...
DUILIB_C_API duilib_container_t duilib_childlayout_to_container(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return nullptr;
...
DUILIB_C_API duilib_childlayout_t duilib_childlayout_from_container(duilib_container_t container) {
    if (!container) return nullptr;
    
    CContainerUI* container_ui = (CContainerUI*)container;
    CChildLayoutUI* childlayout = dynamic_cast<CChildLayoutUI*>(container_ui);
    if (!childlayout) return nullptr;
    
    duilib_childlayout_t result = new(std::nothrow) duilib_childlayout();
    if (result) {
        delete result->layout_ui; // Delete the new one
        result->layout_ui = childlayout; // Use the existing one
    }
    return result;
}
...
DUILIB_C_API duilib_result_t duilib_childlayout_init(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
    
    try {
        layout->layout_ui->Init();
...
//=============================================================================
// XML layout management methods (specific to CChildLayoutUI)
//=============================================================================

DUILIB_C_API duilib_result_t duilib_childlayout_set_child_layout_xml(duilib_childlayout_t layout, const char* xml) {
    if (!is_valid_child_layout(layout) || !xml) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
    
    try {
        CDuiString xml_string = ConvertUTF8ToString(xml);
        layout->layout_ui->SetChildLayoutXML(xml_string);
        return DUILIB_RESULT_OK;
    } catch (...) {
        return DUILIB_RESULT_ERROR;
    }
...
DUILIB_C_API const char* duilib_childlayout_get_child_layout_xml(duilib_childlayout_t layout) {
    ...
    try {
        static thread_local char xml_buffer[4096];
        CDuiString xml_string = layout->layout_ui->GetChildLayoutXML();
        ConvertStringToUTF8(xml_string.GetData(), xml_buffer, sizeof(xml_buffer));
        return xml_buffer;
    } catch (...) {
        return nullptr;
    }
...
DUILIB_C_API duilib_result_t duilib_childlayout_set_attribute(duilib_childlayout_t layout, const char* name, const char* value) {
    if (!is_valid_child_layout(layout) || !name || !value) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_add_control(duilib_childlayout_t layout, duilib_control_t control) {
    if (!is_valid_child_layout(layout) || !control) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_add_control_at(duilib_childlayout_t layout, duilib_control_t control, int index) {
    if (!is_valid_child_layout(layout) || !control) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_remove_control(duilib_childlayout_t layout, duilib_control_t control, bool do_not_destroy) {
    if (!is_valid_child_layout(layout) || !control) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_remove_control_at(duilib_childlayout_t layout, int index, bool do_not_destroy) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API void duilib_childlayout_remove_all(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return;
...
DUILIB_C_API int duilib_childlayout_get_item_index(duilib_childlayout_t layout, duilib_control_t control) {
    if (!is_valid_child_layout(layout) || !control) return -1;
...
DUILIB_C_API int duilib_childlayout_get_count(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return 0;
...
DUILIB_C_API duilib_rect_t duilib_childlayout_get_inset(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) {
...
DUILIB_C_API unsigned int duilib_childlayout_get_child_align(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return 0;
    return layout->layout_ui->GetChildAlign();
}

DUILIB_C_API duilib_result_t duilib_childlayout_set_child_align(duilib_childlayout_t layout, unsigned int align) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API unsigned int duilib_childlayout_get_child_valign(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return 0;
    return layout->layout_ui->GetChildVAlign();
}

DUILIB_C_API duilib_result_t duilib_childlayout_set_child_valign(duilib_childlayout_t layout, unsigned int valign) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_enable_scrollbar(duilib_childlayout_t layout, bool enable_vertical, bool enable_horizontal) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_control_t duilib_childlayout_get_vertical_scrollbar(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return nullptr;
...
DUILIB_C_API duilib_control_t duilib_childlayout_get_horizontal_scrollbar(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return nullptr;
...
DUILIB_C_API duilib_size_t duilib_childlayout_get_scroll_pos(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) {
...
DUILIB_C_API duilib_result_t duilib_childlayout_set_scroll_pos(duilib_childlayout_t layout, duilib_size_t pos) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_size_t duilib_childlayout_get_scroll_range(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) {
        duilib_size_t empty = {0, 0};
        return empty;
    }
...
DUILIB_C_API bool duilib_childlayout_is_visible(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return false;
...
DUILIB_C_API bool duilib_childlayout_is_enabled(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return false;
...
DUILIB_C_API bool duilib_childlayout_is_mouse_enabled(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) return false;
    return layout->layout_ui->IsMouseEnabled();
}

DUILIB_C_API duilib_result_t duilib_childlayout_set_mouse_enabled(duilib_childlayout_t layout, bool enable) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
DUILIB_C_API duilib_result_t duilib_childlayout_set_mouse_child_enabled(duilib_childlayout_t layout, bool enable) {
    if (!is_valid_child_layout(layout)) return DUILIB_RESULT_ERROR_INVALID_PARAMETER;
...
//=============================================================================
// Control search and manipulation methods
//=============================================================================

DUILIB_C_API duilib_control_t duilib_childlayout_find_control(duilib_childlayout_t layout, const char* name) {
    if (!is_valid_child_layout(layout) || !name) return nullptr;
...
DUILIB_C_API int duilib_childlayout_find_selectable(duilib_childlayout_t layout, int index, bool forward) {
    if (!is_valid_child_layout(layout)) return -1;
...
DUILIB_C_API duilib_rect_t duilib_childlayout_get_pos(duilib_childlayout_t layout) {
    if (!is_valid_child_layout(layout)) {
        duilib_rect_t empty = {0};
        return empty;
    }
    ...
    try {
        RECT rect = layout->layout_ui->GetPos();
        return ConvertRect(rect);
    } catch (...) {
        duilib_rect_t empty = {0};
        return empty;
